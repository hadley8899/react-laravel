...
class SendCampaignContactJob implements ShouldQueue
{
    ...
    public function handle(VariableInterpolator $vars): void
    {
        ...
        $html = $vars->interpolate($template->html_cached, $customer);
        $text = $vars->interpolate($template->text_cached, $customer);
        $subject = $vars->interpolate($campaign->subject, $customer);
